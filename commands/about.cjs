...
module.exports = {
    ...
    run: async ({ sock, msg, replyWithTag }) => {
        ...
        try {
            ...
            let pfpUrl;
            try { pfpUrl = await sock.profilePictureUrl(sock.user.id, 'image'); } catch { log('Impossible de récupérer la photo de profil.'); }
            
            if (pfpUrl) {
                await sock.sendMessage(remoteJid, {
                    image: { url: pfpUrl },
                    caption: portfolioText,
                    contextInfo: { externalAdReply: { title: BOT_NAME, body: `Développé par ${CREATOR_NAME}`, sourceUrl: GITHUB_LINK } }
                }, { quoted: msg });
            } else {
